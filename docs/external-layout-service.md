...
The app then realizes that layout by placing the corresponding 3D models from [kitchen element models](../README.md#kitchen-element-models) — the same `KitchenElementDefinition` / `KitchenElementView` / `KitchenLayoutController` pipeline that currently handles manual placement.
